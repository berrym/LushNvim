...
if utils.enabled(group, "telescope") and utils.enabled(group, "lualine") then
	...
	_G.lush_statusline_pick = function()
		...
		local picker = pickers.new({}, {
			...
			attach_mappings = function(prompt_bufnr, _)
				actions.select_default:replace(function()
					...
					local msg = selection.value.label .. (persisted and "" or " (not persisted — add utils.statusline(\"" .. selection.value.name .. "\") to user/config.lua)")
		...
		picker.set_selection = function(self, row)
			...
			local selection = action_state.get_selected_entry()
			if selection then styles_mod.apply(selection.value.name) end
		end
		...
		picker.close_windows = function(status)
			...
			if need_restore then styles_mod.apply(original) end
		end
