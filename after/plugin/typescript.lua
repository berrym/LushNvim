...
if utils.enabled(group, "lsp") then
	...
	vim.api.nvim_create_autocmd("FileType", {
		...
		callback = function(args)
			...
			bind("<leader>ji", function() code_action("source.organizeImports") end, "Organize imports")
			bind("<leader>jm", function() vtsls_exec("typescript.addMissingImports.ts") end, "Add missing imports")
			bind("<leader>ju", function() vtsls_exec("typescript.removeUnused.ts") end, "Remove unused")
			bind("<leader>jF", function() vtsls_exec("typescript.fixAll.ts") end, "Fix all (TS)")
			bind("<leader>jf", function() vim.lsp.buf.format({ async = true }) end, "Format (prettier)")

			-- ESLint
			bind("<leader>jl", function() code_action("source.fixAll.eslint") end, "Fix all (ESLint)")

			-- vtsls-only features
			bind("<leader>jg", function() vtsls_exec("typescript.goToSourceDefinition",
				{ vim.uri_from_bufnr(0), vim.lsp.util.make_position_params(0, "utf-8").position })
			end, "Go to source definition")
			bind("<leader>jR", function() vtsls_exec("typescript.findAllFileReferences") end, "Find all file references")
			bind("<leader>jX", function() vtsls_exec("typescript.restartTsServer") end, "Restart TS server")
			bind("<leader>jv", function() vtsls_exec("typescript.selectTypeScriptVersion") end, "Select TS version")
...
if utils.enabled(group, "package_info") then
	vim.api.nvim_create_autocmd("BufRead", {
		...
		callback = function(args)
			...
			vim.keymap.set("n", "<leader>jp", pi.show, vim.tbl_extend("force", opts, { desc = "Show package versions" }))
			vim.keymap.set("n", "<leader>jP", pi.toggle, vim.tbl_extend("force", opts, { desc = "Toggle package versions" }))
			vim.keymap.set("n", "<leader>jU", pi.update, vim.tbl_extend("force", opts, { desc = "Update package" }))
			vim.keymap.set("n", "<leader>jI", pi.install, vim.tbl_extend("force", opts, { desc = "Install package" }))
			vim.keymap.set("n", "<leader>jd", pi.delete, vim.tbl_extend("force", opts, { desc = "Delete package" }))
			vim.keymap.set("n", "<leader>jc", pi.change_version, vim.tbl_extend("force", opts, { desc = "Change version" }))
		end,
	})
end
	...
	if ok_dap then
		...
		vim.api.nvim_create_autocmd("FileType", {
			...
			callback = function(args)
				...
				bind("<leader>jD", function() dap.continue() end, "Debug (pick config)")
		...
		if vim.fn.filereadable(js_debug) == 0 then
			vim.api.nvim_create_autocmd("FileType", {
				...
				once = true,
				callback = function()
					utils.notify_warn("js-debug-adapter not installed — run :Mason or :LushUpdate", "JS/TS DAP")
				end,
			})
		end
	end
