...
function M.keys()
  ...
  local items = {
    ...
    { icon = " ", key = "n", desc = "New file", action = ":lua require('config.utils').create_new_file()" },
  ...
  if utils.is_git_repo() then
    table.insert(items, { icon = "󰊢 ", key = "g", desc = "Find file in git repo", action = ":Telescope git_files" })
  end

  if utils.enabled(group, "session_manager") then
    table.insert(items, { icon = "󱀸 ", key = "l", desc = "Resume last session",
      action = ":lua require('persisted').load({ last = true })" })
    table.insert(items, { icon = " ", key = "o", desc = "Browse sessions",
      action = ":Telescope persisted" })
  end

  if utils.enabled(group, "project") then
    table.insert(items, { icon = " ", key = "p", desc = "Open project",
      action = ":lua telescope_open_project()" })
  end

  table.insert(items, { icon = "󰋽 ", key = "h", desc = "Health check", action = ":checkhealth lush" })
...
function M.opts(is_enabled)
  if not is_enabled then return { enabled = false } end
  return {
    enabled = true,
    preset = {
      header = M.header,
