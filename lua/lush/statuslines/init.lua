...
M.styles = {
  {
    name = "lush",
    label = "Lush",
    blurb = "Signature LushNvim — language-aware components, the default",
  },
  { name = "evil", label = "Evil", blurb = "Classic eviline" },
  {
    name = "fox",
    label = "Fox",
    blurb = "Nightfox family (carbonfox, duskfox, …) with angled separators",
  },
  { name = "tokyonight", label = "Tokyonight", blurb = "Powerline separators, tokyonight theme" },
  {
    name = "catppuccin",
    label = "Catppuccin",
    blurb = "Rounded bubble sections, catppuccin theme",
  },
  { name = "minimal", label = "Minimal", blurb = "Flat single-line, no chrome" },
  {
    name = "auto",
    label = "Auto",
    blurb = "Adapts to whatever colorscheme is active",
  },
}
...
function M.apply(name)
  ...
  if not ok or type(style) ~= "table" or type(style.setup) ~= "function" then
    vim.notify(
      "Unknown statusline style: " .. tostring(name) .. " — falling back to " .. M.default,
      vim.log.levels.WARN,
      { title = "LushStatusline" }
    )
    name = M.default
