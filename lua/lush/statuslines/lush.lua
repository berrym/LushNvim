...
local function go_module()
  if vim.bo.filetype ~= "go" and vim.bo.filetype ~= "gomod" then
    return ""
  end
  if vim.b.lush_go ~= nil then
...
local function c_build_system()
  ...
  if vim.b.lush_cbuild ~= nil then
    return vim.b.lush_cbuild
  end
  local root =
    find_root({ "meson.build", "CMakeLists.txt", "Makefile", "configure.ac", "configure.in" })
  if not root then
    vim.b.lush_cbuild = ""
    return ""
  end
  local tag = (vim.fn.filereadable(root .. "/meson.build") == 1 and "󰫴 meson")
